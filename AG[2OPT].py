...
def nextGeneration(graphe, population, parents_size, eliteSize, SelectionAlgo, nbPointCroisement, probaMutation,
    ...
    # 2OPT
    intense_population = solve_tsp_2opt(graphe, mutated_fils)

    new_population = np.array(np.append(parents, intense_population, axis=0))
...
def LRH_AG_2opt(graphe, population_size, nbgenerations, parents_size, eliteSize=2, genAlgo="Random",
...
if __name__ == '__main__':
    ...
    tour, cost = LRH_AG_2opt(np.array(instance.data), population_size=int(args.population_size),
